...
import com.quickfixes.edt.core.BlankLinesFixer;
import com.quickfixes.edt.core.MarkerScanner;

/**
 * "Quick Fixes → Удаление неиспользуемого → Удалить пустые строки" — collapses runs of consecutive
 * blank lines flagged by {@code module-consecutive-blank-lines} down to a single blank line.
 */
public class DeleteBlankLinesHandler extends AbstractHandler
{
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException
    {
        ...
        Set<Integer> flaggedLines =
            MarkerScanner.linesForCheck(markerManager, checkRepository, project, file, BlankLinesFixer.CHECK_ID);
        if (flaggedLines.isEmpty())
        {
            MessageDialog.openInformation(shell, TITLE,
                "В текущем модуле нет ошибок проверки «module-consecutive-blank-lines»."); //$NON-NLS-1$
            return null;
        }

        BlankLinesFixer.Plan plan = BlankLinesFixer.collapse(document, flaggedLines);
        if (plan.removed == 0)
        {
            MessageDialog.openInformation(shell, TITLE, "Нечего удалять."); //$NON-NLS-1$
            return null;
        }
        try
        {
        ...
        {
            Activator.logError("Failed to collapse blank lines", e); //$NON-NLS-1$
            MessageDialog.openError(shell, TITLE, "Не удалось применить: " + e.getMessage()); //$NON-NLS-1$
            return null;
        }
        MessageDialog.openInformation(shell, TITLE, "Удалено пустых строк: " + plan.removed); //$NON-NLS-1$
        return null;
    }
}
