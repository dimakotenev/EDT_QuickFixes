...
/**
 * "Quick Fixes → Исправление устаревших конструкций → Исправление излишнего обращения к ЭтотОбъект" —
 * removes the redundant self-reference prefix ({@code X.Member} &rarr; {@code Member}) for
 * {@code module-self-reference}, {@code common-module-named-self-reference} and
 * {@code manager-module-named-self-reference}.
 */
public class FixRedundantSelfReferenceHandler extends AbstractHandler
{
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException
    {
        ...
        final List<MarkerScanner.MarkerEntry> entries = MarkerScanner.collect(markerManager, checkRepository,
            project, file, SelfReferenceFixer.REDUNDANT_CHECK_IDS);
        if (entries.isEmpty())
        {
            MessageDialog.openInformation(shell, TITLE,
                "В текущем модуле нет ошибок избыточного обращения к самому себе."); //$NON-NLS-1$
            return null;
        }

        SelfReferenceFixer.Plan plan =
            document.readOnly(resource -> SelfReferenceFixer.fixRedundant(document, resource, entries));
        if (plan.count == 0)
        {
        ...
        {
            Activator.logError("Failed to fix redundant self-reference", e); //$NON-NLS-1$
            MessageDialog.openError(shell, TITLE, "Не удалось применить: " + e.getMessage()); //$NON-NLS-1$
            return null;
        }
        new ResultDialog(shell, "Исправление излишнего обращения к ЭтотОбъект", //$NON-NLS-1$
            "Исправлено: " + plan.count, "Конструкция", "Действие", plan.result).open(); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        return null;
    }
}
